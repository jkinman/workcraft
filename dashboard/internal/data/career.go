...
// Loaded from templates/tracker-contract.json (generated by lib/tracker/contract.mjs).
func trackerHeaderAliases() map[string]string {
	aliases, err := TrackerHeaderAliases()
	if err != nil {
		// Fallback mirrors the last known contract so a parse failure does not brick the dashboard.
...
// UpdateApplicationStatusAndNotes atomically updates tracker status, report
// frontmatter, and status-log.tsv via lib/tracker/cli-transition.mjs.
func UpdateApplicationStatusAndNotes(careerOpsPath string, app model.CareerApplication, newStatus, notesAppend string) error {
	return runCanonicalTransition(careerOpsPath, app.ReportNumber, newStatus, notesAppend, "go-dashboard")
}
