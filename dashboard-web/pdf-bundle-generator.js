...
function makeFilename(type, company) {
  const date = new Date().toISOString().split('T')[0];
  const companySlug = company.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  return `${type}-joel-kinman-${companySlug}-${date}.pdf`;
}
...
<div class="page">
  ...
  <div class="doc-footer">
    Generated by Joel Kinman's AI Assistant // ${company} — ${role} // ${new Date().toLocaleDateString()}
  </div>
</div>
</body>
</html>`;
...
<div class="page">
  ...
  <div class="doc-footer">
    Generated by Joel Kinman's AI Assistant // ${company} — ${role} // ${today}
  </div>
</div>
</body>
</html>`;
...
<div class="page">
  <div class="doc-header">
    ...
    <div>
      ...
      <div class="contact-row">
        Generated by Joel Kinman's AI Assistant // ${new Date().toLocaleDateString()}
      </div>
    </div>
  </div>
  ...
  <div class="doc-footer">
    Joel Kinman's AI Assistant // Score: ${job.score}/5.0 // ${job.company}
  </div>
</div>
</body>
</html>`;
...
<div class="page">
  <div class="doc-header">
    ...
    <div>
      ...
      <div class="contact-row">
        Generated by Joel Kinman's AI Assistant // ${new Date().toLocaleDateString()} // CONFIDENTIAL
      </div>
    </div>
  </div>
  ...
  <div class="doc-footer">
    Joel Kinman's AI Assistant // Score: ${job.score}/5.0 // Verdict: ${job.verdict || 'PENDING'}
  </div>
</div>
</body>
</html>`;
...
async function generateResumePDF(company, role, jobDescription) {
  try {
    const cv = parseCV();
    ...
    const outputDir = ensureOutputDir();
    const filename = makeFilename('cv', company);
    const outputPath = path.join(outputDir, filename);
...
async function generateCoverLetterPDF(company, role, jobDescription) {
  try {
    const cv = parseCV();
    const profile = loadProfile();
    const format = detectFormat(jobDescription + ' ' + company);
    ...
    const outputDir = ensureOutputDir();
    const filename = makeFilename('cover-letter', company);
    const outputPath = path.join(outputDir, filename);
...
async function generateEvalReportPDF(job, rawMarkdown) {
  try {
    const format = detectFormat(job.location + ' ' + job.company);

    const html = buildEvalReportHTML(job, rawMarkdown, format);
    const outputDir = ensureOutputDir();
    const filename = makeFilename('eval-report', job.company);
    const outputPath = path.join(outputDir, filename);
...
async function generatePDFBundle(company, role, jobDescription, job, rawMarkdown) {
  const results = await Promise.all([
    generateResumePDF(company, role, jobDescription),
    generateCoverLetterPDF(company, role, jobDescription),
    generateEvalReportPDF(job, rawMarkdown)
  ]);
